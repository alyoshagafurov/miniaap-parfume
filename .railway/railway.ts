...
export default defineRailway(() => {
   ...
   *
   * Имя региона — КОНКРЕТНОЕ, а не псевдоним, и это не вкусовщина.
   *
   * `railway service scale` принимает псевдонимы (`us-west`, `us-east`,
   * `eu-west`, `southeast-asia`) и раскрывает их сам. Этот файл их НЕ
   * раскрывает: записанный сюда `eu-west` так и уходит в `multiRegionConfig`,
   * а планировщик Railway такого региона не знает — деплой уходит в FAILED
   * вообще без инстансов и без единой строчки логов, что выглядит как отказ
   * самой базы. На это ушло три круга: postgres и redis падали именно так,
   * пока web, которому регион ставили через `scale`, спокойно жил.
   *
   * Рядом живут ещё два списка имён, и путать их легко: в GraphQL-справочнике
   * регион зовётся `europe-west4`, у бакетов список из четырёх и Амстердам там
   * `ams`, а у сервисов на Railway Metal встречается `sfo`.
   *
  ...
  const web = service("web", {
    ...
    env: {
      ...
      // Вход в админку одним паролем, без кода из Telegram. Попрошен клиентом;
      // по умолчанию код требуется, и вернуть его — это "1" здесь и передеплой.
      // Цена решения записана в decideLogin: этот пароль остаётся единственной
      // защитой цен, публикации товаров и заявок с телефонами покупателей.
      ADMIN_LOGIN_REQUIRE_CODE: "0",
    },
  });
