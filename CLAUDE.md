...
**Основная — Railway.** Четыре сервиса в одном проекте: web, bot, Postgres,
Redis, плюс Bucket под фотографии. Домен, сертификат и HTTPS — от платформы.
Бот ходит в `api.telegram.org` напрямую, релей не нужен. Описание —
[.railway/railway.ts](.railway/railway.ts), порядок действий —
[docs/DEPLOY.md](docs/DEPLOY.md).

**Запасная — VPS в РФ.** Остаётся рабочей на случай, если из России витрина
будет открываться медленно: `docker-compose.prod.yml`, `Caddyfile`, `relay/`,
`scripts/deploy.sh`, `scripts/backup.sh`, `scripts/restore.sh`. Там релей
обязателен — `api.telegram.org` с российского хостинга недоступен.

Что различается в коде: ничего, кроме переменных. `DEPLOY_TARGET=vps`
включает в `pnpm deploy:check` проверки домена и HSTS, которые на Railway
не имеют смысла.

**Конфигурация Railway — `.railway/railway.ts`, не `railway.toml`.** Config as
Code объявлен устаревшим: файлы перестают читаться 2026-12-01, а новые сервисы
в него уже не пускают. И `tsc`, и `eslint` пропускают каталоги, начинающиеся с
точки, — то есть этот файл нашими проверками не покрыт, его валидирует
`railway config plan`.

**Фотографии: Railway Bucket закрытый, публичных бакетов платформа не
поддерживает.** Поэтому `NEXT_PUBLIC_S3_PUBLIC_URL` там пуст, а браузер берёт
картинки через `/api/media/…` — прокси, который читает объект ключами бакета и
отдаёт с `immutable`-кэшем на год (ключи содержат хэш содержимого, устареть не
могут). Заполненный `NEXT_PUBLIC_S3_PUBLIC_URL` переключает на прямую отдачу из
публичного бакета, и прокси перестаёт использоваться. Пустое значение — рабочая
конфигурация, а не забытая переменная.

Presigned-ссылки не подошли: они истекают, а адреса картинок попадают в HTML,
который живёт в `'use cache'` часами, и в корзину в localStorage, которая живёт
днями.
...
**`prisma generate` требует `DATABASE_URL`, хотя база ему не нужна.**
`prisma.config.ts` объявляет `datasource: { url: env("DATABASE_URL") }`, а
`env()` вычисляется при загрузке конфига и бросает, если переменной нет. При
сборке образа базы нет и быть не должно — поэтому в Dockerfile стоит
`ARG DATABASE_URL=…` с заведомо нерабочим адресом. Именно `ARG`, а не `ENV`:
`ARG` не попадает в окружение готового образа, иначе забытый в compose
`DATABASE_URL` молча подменился бы заглушкой вместо отказа стартовать.

**`prisma.config.ts` зовёт `loadEnvFile` в `try`.** `?.` защищает от
отсутствия функции, а не файла: без `.env` вызов бросает ENOENT. В контейнере
`.env` нет — переменные приходят из окружения, — и без `try` каждый
`prisma migrate deploy` падал до того, как прочитает первую миграцию.

**`next build` требует живой базы, и границы Suspense этого не меняют.** Под
`cacheComponents` сборка выполняет функции с `'use cache'`, чтобы наполнить кэш
при пререндере, — то есть по-настоящему ходит в PostgreSQL. Граница решает, где
окажется результат, а не будет ли он вычислен при сборке. На Railway лечится
`DATABASE_PUBLIC_URL`, на своём сервере — порядком в `scripts/deploy.sh`.

**`.env.production` в корне перекрывает `.env` для `next build`.** Next в
production читает `.env.production` раньше `.env`, поэтому файл с продовыми
значениями в корне ломает локальную сборку: она идёт в хост `postgres`, которого
на машине нет. На сервере это не проявляется — образ собирается внутри Docker,
где `.env*` исключён через `.dockerignore`. Локально для проверки compose
используйте другое имя (`.env.prodtest`) и передавайте его через `--env-file`.

### Отклонения от брифа

Каждое — сознательное, с причиной. Если причина отпадёт, отклонение тоже.

**Таблица товаров без TanStack Table.** Бриф называет её в блоке 9. Таблица
серверная: страница по 50, сортировка, фильтры и поиск считаются в PostgreSQL,
состояние живёт в URL — «весь каталог в браузер не грузим» из дополнения №4.
TanStack Table решает задачу клиентской таблицы: сортировку и фильтрацию над
массивом, который уже в памяти. Здесь этого массива нет и быть не должно, и
библиотека добавила бы к витрине вес ради API, от которого используется разве
что разметка `<thead>`. Согласовано с заказчиком.

**JS первого экрана 177–180 КБ gzip против 150 КБ в блоке 10.** react-dom 71 +
рантайм app-router 35 + `@tma.js/sdk` 27 = 133 КБ до первой строки приложения.
Внутри блока 9 (Next 16 + React 19 + `@tma.js`) в 150 не уложиться. Числа и
единственный оставшийся резерв — в
[qa/stage-1/lighthouse/README.md](qa/stage-1/lighthouse/README.md).

**LCP 2,6–2,8 с на категории и карточке против 2,5 с.** На главной 2,1 с.
Та же причина и тот же резерв.

**`/p/<нет-такого>` отвечает 200, а не 404.** И `/c/`, и `/b/` — тоже. Под PPR
статическая оболочка уходит раньше, чем выполняется запрос в базу, поэтому
`notFound()` уже не может изменить строку состояния. Отказаться от PPR ради
кода ответа — значит отнять у каждого настоящего товара мгновенный скелет на
связи рыночного павильона; для каталога, который читают внутри Telegram, обмен
неверный. Человек при этом видит нормальный экран
([src/app/(shop)/not-found.tsx](<src/app/(shop)/not-found.tsx>)), а не пустоту.
Лечится в этапе 3 на уровне релея — там же, где SEO.

**`meta-description` и `<title>` категории и карточки приходят в `<body>`.**
`generateMetadata` ждёт `params`, под `cacheComponents` это динамика. Браузер
поднимает их в `<head>`, краулер без JS — нет. Этап 3, блок 3.

**Lighthouse на этом профиле даёт разброс ±0,7 с по LCP.** Один и тот же билд
на двух подряд прогонах главной дал 2,1 с (perf 99) и 2,8 с (perf 96). Числа в
отчётах — медиана трёх прогонов; одиночный замер здесь ничего не значит.

**Скриншоты Playwright не в git.** `/qa/**/*.png` в `.gitignore`. Этап 2 — это
визуальный слой: девяносто изображений, переписываемых целиком после каждого
прохода по цвету, за несколько недель весят больше всего остального репозитория.
Создаются `pnpm test:e2e` в `qa/final/screens/`. Исключение — девять
иллюстраций к [docs/HANDOFF.md](docs/HANDOFF.md) в `qa/final/handoff/`: это
часть документа для клиента, а не улика прогона, и они в git есть.
