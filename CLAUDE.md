...
pnpm check:build-io   страж: сборка не читает базу
...
Направление «Белая сцена» — по референсу, который закрепил клиент. Система
записана в [DESIGN.md](DESIGN.md) (токены во frontmatter, правила в тексте);
значения — в [src/app/globals.css](src/app/globals.css), единственном источнике.
Прежнее «Оливковый шёлк» заменено целиком: не возвращать кремовый фон, засечный
дисплей, центрированные подписи между линиями и рамки вокруг каждой карточки.

Коротко: товар на белой сцене (`stage`, радиус 28) на нейтральном фоне
`#F2F2F0`; заголовки — Manrope 800 капсом (`display-caps`); всё, что нажимается,
— капсула; тёмные блоки (`bg-night`) — внутри светлых экранов, не тёмная тема.
Cormorant остался только в знаке ÁRUMI и монограмме Á, олива — только там же.

**Янтарь — только деньги, и янтарей два.** `--color-price` на светлом фоне
(4.6:1), `--color-price-bright` только на тёмном блоке (6.3:1): яркий янтарь
референса на белом даёт 2.65:1 и на солнце пропадает. Ссылки, вкладки и
активные состояния янтарём не красятся. `src/lib/tokens.test.ts` меряет контраст
каждой пары прямо по стилям и падает, если цвет уехал.

`pnpm check:tokens` — страж. Сброс пространств Tailwind (`--color-*: initial`)
убирает подмену палитры, но **ничего не запрещает**: в v4 неизвестная утилита
просто не генерирует правило, а `bg-[#ff0000]` компилируется. Отсюда же правило
`undefined-text-size`: `text-h2` стоял в пятнадцати местах и не был определён
ни разу — заголовки админки и итог заявки рендерились унаследованным размером.
...
Золото — только линии (подчёркивание текущего формата и вкладки, линия под
шапкой). Текстом быть не может.

Число не отрывается от единицы: «35 мл», «2 в 1» склеиваются неразрывным
пробелом через `keepUnits()` из `src/lib/format.ts` везде, где выводится
название категории.
...
Скилл `impeccable` ведёт свои файлы в `.impeccable/`: бриф поверхности и
`config.json` (путь сборки `code` — генерации изображений здесь нет) лежат в
git, снимки проверки и состояние сборки — нет.
...
**Приватная сеть Railway недоступна при сборке** — и это больше ни на что не
влияет, потому что сборка в базу не ходит (см. ниже). Публичный доступ к
Postgres включать не нужно, и `DATABASE_PUBLIC_URL` нигде не объявлен.
...
**`next build` ходил в базу, и границы Suspense этого не меняли.** Под
`cacheComponents` пререндер ВЫПОЛНЯЕТ функции с `'use cache'`, чтобы наполнить
их записи; граница решает, где окажется результат, а не будет ли он вычислен.
Лечилось публичным адресом базы, и цена была тройная: первый деплой падал, так
как миграции идут после сборки; пароль оставался в истории слоёв образа; базу
приходилось открывать в интернет.

Решение — `io()` из `next/cache`, точка приостановки **до** запроса. Каждое
чтение витрины экспортируется парой: публичная функция ждёт `io()` и зовёт
приватную, на которой висит `'use cache'`. Две функции, а не одна, потому что
внутри кэш-области `io()` — no-op по определению. Теги, `cacheLife` и
`updateTag` не меняются; запись просто наполняется при первом настоящем
запросе, а не при сборке.

`io()`, а не `connection()`: последний ждёт настоящей навигации и тем самым
ломает префетч. Исключение одно — `/api/health`, где ждать настоящего запроса
и надо. Его пришлось чинить отдельно: `next build` выполняет route handlers,
проверяя, нельзя ли их сделать статикой, и этот отвечает 503 вместо броска —
503, впечатанный в сборку, это контейнер, который никогда не пройдёт
собственную проверку живости.

Проверяется одной командой: сборка с заведомо нерабочим `DATABASE_URL` должна
пройти и не напечатать ни одной ошибки Prisma.
